...
def post_process(case_dir, sim_id, *, velocity, rho=1025.0, p_vapour=None):
    ...
    try:
        if vol is not None and "Q_criterion" in vol.array_names:
            # Q-criterion for tip vortices: use higher multiplier to capture only
            # strong vortex cores. Values 10-50x (V/chord)^2 isolate tight spirals.
            Q_threshold = 10.0 * (V / chord_length) ** 2
            iso = vol.contour(isosurfaces=[Q_threshold], scalars="Q_criterion")

            # Adaptive threshold: try lower if empty, higher if too dense
            if iso is None or iso.n_points == 0:
                Q_threshold *= 0.25
                iso = vol.contour(isosurfaces=[Q_threshold], scalars="Q_criterion")
            elif iso.n_points > 50000:
                Q_threshold *= 3.0
                iso = vol.contour(isosurfaces=[Q_threshold], scalars="Q_criterion")
...
def run_hydro_simulation(self, sim_id):
    ...
    try:
        ...
        # Ensure feature refinement grows with mesh density to reduce coarse “blocky” appearance.
        ...
        if os.path.exists(stl_path):
            ...
            try:
                ...
                # Mesh resolution heuristic: allow up to ~40 cells across the characteristic
                # length at mesh_density=1.0, and 80+ cells for mesh_density=2.0.
                # Keep a floor to avoid numerical instabilities on very tiny models.
                effective_cells = 40.0 * mesh_density
                cell_size = max(characteristic_len / effective_cells, 0.0005)
                lx = max(domain["x_max"] - domain["x_min"], 1e-9)
                ly = max(domain["y_max"] - domain["y_min"], 1e-9)
                lz = max(domain["z_max"] - domain["z_min"], 1e-9)

                mesh_cells = {
                    "nx": max(30, min(500, int(lx / cell_size))),
                    "ny": max(20, min(400, int(ly / cell_size))),
                    "nz": max(20, min(400, int(lz / cell_size))),
                }
                ...
                # Increased cap to better support high mesh_density values (2.0) with more realistic refinement.
                max_base_cells = 800_000
                ...
                if base_cells > max_base_cells:
                    ...
                    mesh_cells = {
                        "nx": max(30, int(mesh_cells["nx"] * scale)),
                        "ny": max(20, int(mesh_cells["ny"] * scale)),
                        "nz": max(20, int(mesh_cells["nz"] * scale)),
                    }
        ...
        # 5. Phase 5: Temporal Frame Extraction (VTP with Cp per time step)
        ...
        if forces_by_time:
            ...
            for idx, t in enumerate(sorted_times):
                ...
                fz = entry.get("Fz")
                ld = None
                if fx and abs(fx) > 1e-12 and fz is not None:
                    ld = abs(fz / fx)
                metrics_list.append({
                    "frame_index": idx,
                    "time_value": t,
            ...
            completed_payload["metrics_series"] = metrics_list
        ...
        if temporal_frame_mapping and forces_by_time:
            ...
            for frame in temporal_frame_mapping:
                ...
                if fm.get("Fx") is not None:
                    continue  # already has valid metrics from Cp integration
                tv = frame.get("time_value")
                if tv is None:
                    continue
                tv = float(tv)
                # Find nearest force time
